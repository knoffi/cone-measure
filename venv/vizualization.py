...
def vizConeDataParamFunction(size, stepSize, function , cD, param, value_1, value_2, value_3):
    ...
    mp.plot(x_1, y_1, 'go' , markersize = const_marker)
    mp.plot(x_2, y_2, 'yo' , markersize = const_marker)
    mp.plot(x_3, y_3, 'ro' , markersize = const_marker)
    mp.plot(x_4, y_4, 'bo' , markersize = const_marker)
    #mp.plot([point_min[0]], [point_min[1]], 'go')
    mp.axes([a_x, b_x, a_y, b_y])
    ...
    #print( cV.phi( point_min , cD ))
    #print( cV.gradPhi( point_min , cD))
    mp.savefig('trapezMin0001x001x01.png')
    mp.show()
...
const_marker = 1
P_testGrafic = [ [ 1,1 ], [-1,1] , [-1,-1 ], [2,-1]]
cD_testGrafic = cV.getConeVol(P_testGrafic)
vizConeDataParamFunction(0.1, 0.0001, cV.polyFunct2,cD_testGrafic,[1,1],0.001,0.01,0.1)
